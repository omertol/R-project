...
ldf <- sapply(list_df, read.csv, simplify = FALSE)
```
...
ldf <- sapply(ldf, remove_inactive, simplify = FALSE)
ldf <- sapply(ldf, sep_wells, simplify = FALSE)
```
...
```{r ANOVA tests and Tukey’s Test for multiple comparisons}
#There is no interaction between the 3 factors - H0 
anov1 <- aov(corr ~ treatment * time_p * as.factor(distance), tmp)
summary(anov)
TukeyHSD(anov1)
 
anov2 <- aov(corr ~ treatment * time_p + as.factor(distance), tmp)
summary(anov)
TukeyHSD(anov2)
```
```{r}
library(AICcmodavg)
model.set <- list(two_way, interaction, anov1,anov2)
model.names <- c("two.way", "interaction", "anov_stav", "anov_omer")
