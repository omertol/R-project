...
corr_df <- corr_df %>% group_by(file_id, treatment, well_id, distance) %>%
...
# fig1_data <- data.frame(x=two_way_model$fitted.values, 
#                         y=two_way_model$residuals,
#                         stres= rstandard(two_way_model),
#                         )
# 
# fig1 <- fig1_data %>% ggplot(aes(x = x, y=y)) +
#   geom_point() + 
#   labs(
#     x= "Fitted Values",
#     y= "Residuals",
...
two_way_model <- lm(corr ~ treatment * time_p, tmp)
par(mfrow = c(1,2))
plot(two_way_model, c(1,2), add.smooth = FALSE)
```
...
three_way_model <- lm(corr ~ treatment * time_p + distance, tmp)
par(mfrow = c(1,2))
plot(three_way_model, c(1,2), add.smooth = FALSE)
```

```{r Tukey’s Test for multiple comparisons}
tbl1 <- as.data.frame(TukeyHSD(three_way_interaction, which = c("treatment"))[[1]])

tbl2 <- as.data.table(TukeyHSD(three_way_interaction, which = c("treatment:time_p"))[[1]], 
                                  keep.rownames = TRUE)
...
tbl2 <- tbl2[treatment1 == treatment2, c(1,2,4,5,6,7,8)]

tbl1
tbl2
```

```{r Graph per treatment, fig.height=2, fig.width=7}
fig3_data <- tmp %>% 
  group_by(time_p, treatment, distance) %>% 
  summarise_at(vars(corr), mean)

fig3 <- ggplot(transform(fig3_data,
                         treatment = factor(treatment,levels = c("Control","AP5","FGL")),
                         time_p = factor(time_p,levels = c("Before","During","After"))),
               aes(x=distance,
                   y=corr,
                   color=time_p,
                   group=time_p)) +
