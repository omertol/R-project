...
if (file.exists("./data/wells distance.csv")){
  ...
  well_dist <- as.data.frame(lapply(well_dist, as.numeric))
} else {
...
anov_ <- cbind(x,y)
colnames(anov_) <- c("Well", "file_id", "treatment", "elec1","elec2", "distance","corr")
anov_[is.na(anov_)] <- 0

anov_ <- anov_ %>% group_by(file_id, treatment, distance) %>% summarise_at(vars(corr), mean)
```
