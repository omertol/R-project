...
library(hrbrthemes)
library(viridis)
tmp <- tmp %>% group_by(time_p, treatment, distance) %>% summarise_at(vars(corr), mean)

ggplot(transform(tmp,treatment=factor(treatment,levels = c("Control","AP5","FGL")),time_p=factor(time_p,levels = c("Before","During","After"))), aes(x=distance, y=corr,color=time_p,group=time_p)) +
geom_line()+facet_wrap(~treatment)+theme_ipsum()+labs(x="Distance(μm)" ,y="Correlation", color="Time")
```
