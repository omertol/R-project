...
## 2. Data
The data contains records of: time, Petri number, Electrode number, Amplitude.
...
The following graph shows the difference between electric signal sent during the day for a specific petri dish called "D4". 
...
The following graph shows the amplitude of the signals sent from one neuron during the day, as recorded by the electrode its on, for the same petri dish.
Using the boxplot graph we can learn the maximum, mean and minimum of the voltage sent per electrode.

The next table is the mean values of the amplitude discussed above.
